...
image::threads.jpg[Photo by Wendy van Zyl, 700, role="right"]
...
image::toaster.jpg[Photo by Claudio Schwarz, 700, role="right"]
...
[graphviz, "utilization.png"]
-----
digraph Q {
  rankdir=LR
  bgcolor="transparent";
  node [shape=record style="setlinewidth(0)"];
  sequencial [label = <
...
Each thread consumes memory for the stack, number of threads physically limited by machine memory.
Systems like Django and Flask create a thread for each client.
...
You can create a lot of threads, but only limited number of them will work in the same time. Limit is the number of CPU.
...
Each sockei is a file for OS, so we are limited by number of file handlers.
...
* **Promises (Futures)**
  When you start async task you get an object that know about your task. You can ask it if you task is ready.
...
you can write onw implementation on pure Python.

It is a single point of failure. If you loop is dead, all coroutines are dead.


== Dummy event loop
[source,python, source-highlighter=highlightjs]
.loop.py
----
...
.Step by step
|===
| asyncio.run( | event loop started
| asyncio.run(main()) | coroutine added to event loop queue,
control returned to event loop, event loop starts the coroutine (main))
|print(f"started at {time.strftime('%X')}")| prints text in the main function
|say_after(1, 'hello')| coroutine added to event loop queue,
control passed to event loop, event loop starts the coroutine (say_after(1, 'hello')))
|asyncio.sleep(delay)| coroutine added to event loop queue,
control returned to event loop, event loop is looping
|await asyncio.sleep(delay)| event loop wait until coroutine finished and control passed to function
|print(what)| do some stuff with IO and exit function, It is await function and control passed to event loop after it finished.
| await say_after(1, 'hello') | loop see that it can return control to main function
| await say_after(2, 'world') | once more schedule, wait until finished.
| print(f"finished at {time.strftime('%X')}") | print to IO and finish. Event loop closed.
|===

Do you notice that if we remove asyn/await nothing will be changed? Program will still do the same in the same amount of time?
...
This code do a magic trick, all tasks in the queue started when first await is called.

`await task1` will start both task1 and task2. `await say_after(1, 'oops')` is not a task,
so this will not trigger any task start.
...
== Async server
If you wan a server that can hold millions of the simple requests, async server is your choice.
Speed come in a price: you will need to have all tech to be async. So you will be limited.
...
Not all databases has async client that is supported by SQLAlchemy. Async code is also harder to write and maintain.

There is a lot of them. I would say that the list of features they provide looks more like a Flask than a Django.
Most of them have integration with SQL Alchemy.


=== https://www.tornadoweb.org/en/stable/[Tornado] it is a server and a client.
It was designed more than 10 years ago, before async/await pop up.
Almost the same idea, but generators instead of `def async` and `yeild` instead od `await`.
Thy created own async web client to work with that event loop.
Now Tornado supports async/await and generator workflow deprecated.


=== https://github.com/tiangolo/fastapi[FastAPI]
Another example is is FastAPI, known as fastest framewrok alive (by FastAPI)
...
== Async client
You can wrap you `requests` call to async function, but it would be a blocking call.
Better than sequential, but not as good as pure asynchronous.
Probably you will not any difference, since there are some optimizations when `urllib3` works with sockets.
...
== Async ORM
https://github.com/python-gino/gino[gino], GINO Is Not ORM - is a lightweight asynchronous ORM built on top of SQLAlchemy core for Python asyncio.
...
- async approach is better utilize resources when task is IO bound, it will work more effective under heavy load.
...
Async in js  https://eloquentjavascript.net/11_async.html
